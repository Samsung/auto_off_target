...
import logging
import hashlib
from aotdb_api import AotDbCollectionItem
from aotdb_api import AotDbCollectionQuery
from aotdb_api import AotDbCollections
...
try:
    ...
    print("Unable to import libftdb")

class FtdbBackend(AotDbFrontend):
    ...
    def establish_db_connection(self, args):
        # get the JSON file
        self.jsondb = FtdbJSON(args.import_json, 
                          args.product,
                          args.version,
                          args.build_type,
                          args.drop_on_import,
        ...
        if not self.jsondb.sanity_check():
            logging.error("Parameters sanity check failed. Exiting.")
            sys.exit(1)

        if not self.jsondb.connect():
...
class FtdbCollectionItem(AotDbCollectionItem):
...
class FtdbJSON(AotDbCollections):
    ...
    def sanity_check(self):
        ...
        if self.db_file is None and self.json_file is None:
            logging.error(f"Must specify either --json-file (import) or --db (normal use)")
            return False
    ...
    def _query(self, visited, collection_name, base, match_from_field, match_to_field, value_to_return=None, cutoff_list=None):
        ...
        if isinstance(from_obj, list):
            for item in from_obj:
                ...
                results = self.collections[collection_name].find(match_to_field, item)
                for r in results:
            ...
            results = self.collections[collection_name].find(match_to_field, from_obj)
            for r in results:
    ...
    def make_recursive_query(self, collection_name, obj_selector_field, obj_selector_value, match_from_field, match_to_field, value_to_return=None, add_vals=None, cutoff_list=None):
        ...
        base = self.collections[collection_name].find_one(obj_selector_field, obj_selector_value)
    ...
    def connect(self):
        ...
        collection_names = [ "BAS", "funcdecls", "funcs", "funcs_tree_calls_no_asm",
        ...
        self.collections = {name: FtdbCollectionItem(name, self.db, "id") 
