...
/* Just a simple wrapper to match function prototype with the one expected by flatten_load*/
...
void aot_kflat_init(const char* imgpath) {
	...
	flatten = flatten_init(0);
	assert(flatten != NULL);

	ret = flatten_load(flatten, in, get_fpointer_test_function_address);
	assert(ret == 0);
...
void* aot_kflat_root_by_name(const char* name, unsigned long* size) {
	return flatten_root_pointer_named(flatten, name, size);
}
